...
*/
class Solution {
    private void createNode(Node node,Map<Node,Node> map,Set<Node> visited){
        visited.add(node);
        map.put(node,new Node(node.val));
        for(Node neighbour:node.neighbors){
            if(!visited.contains(neighbour)) createNode(neighbour,map,visited);
        }
    }
    Node cloneGraph(Node node) {
        ...
        createNode(node,map,visited);
        
        for(Node original:map.keySet()){
            for(Node n:original.neighbors){
                map.get(original).neighbors.add(map.get(n));
            }
        }
