...
class Solution
{
    ...
    public ArrayList<Integer> bottomView(Node root)
    {
        ...
        // TreeMap to store the bottom view nodes with their HDs
        ...
        while (!queue.isEmpty()) {
            ...
            // Update the map with the current node's value for its HD
            map.put(hd, node.data);

            // Add left child to the queue with its HD
            if (node.left != null)
                queue.offer(new Pair<>(node.left, hd - 1));

            // Add right child to the queue with its HD
            if (node.right != null)
                queue.offer(new Pair<>(node.right, hd + 1));
        }

        // Extract values from TreeMap (bottom view sorted by HDs)
